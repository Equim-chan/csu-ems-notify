...
mailOptions.html += '<br>csu-ems-api返回的JSON：<br><div style="font-size: 12px;background:#efefef;padding:8px;">';
mailOptions.subject = 'csu-ems-notify测试结果';

var start = new Date();
console.log(util.format('    (0ms) Testing csu-ems-api... (url: %s)'.cyan, api));
superagent
    .get(api + '/grades')
    ...
    .end(function (err, res) {
        ...
        console.log(util.format('    (' + (new Date() - start) + 'ms) Testing nodemailer... (host: %s)', config['sender-options'].host).cyan);
